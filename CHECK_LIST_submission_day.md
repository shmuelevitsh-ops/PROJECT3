...
- **PART A — Final whole-project code scan:** readability, coding-quality risks, critical safety issues, comment cleanup, and Known Issues preparation.
...
- Do **not** review tests.
- Do **not** fix or refactor tests.
- Do **not** clean comments in tests.
...
4. Course grading/error-code guidance.

---

# PART A — FINAL WHOLE-PROJECT CODE SCAN

Run once over all **student-owned production code**.

Goal: find only issues worth acting on before submission.  
This is **not** a learning review and **not** a general refactoring pass.

## A1. Grading/code-quality risks

Check for the course error-code issues that are relevant to source code:

- [ ] `e01/e02` — bad header/class organization.
- [ ] `e03/e16` — incorrect `mp-units` use or replacing project strong types with plain numerics.
- [ ] `e04` — incorrect C++ standard-library usage.
...
- [ ] `e09` — clearly overlong functions.
- [ ] `e10` — duplicated class/function/logical flow.
- [ ] `e11/e17` — unapproved/manual dependency or unnecessary dependency/include.
- [ ] `e13` — incorrect pointer use.
- [ ] `e21` — meaningful repeated expensive computation.
- [ ] `e23` — meaningful magic numbers that should be named/configured.

Only flag **real, actionable** issues.  
Do not report optional redesigns or stylistic preferences.
...
- [ ] obvious data races or plausible deadlocks;
...
- lifetime/ownership constraints;
- synchronization/concurrency reasoning;
- a decision that cannot be understood from the code itself.

Remove:
- comments that restate the next line;
- tutorial-style C++ explanations;
- narrative descriptions of straightforward control flow;
- excessive block/function documentation;
- stale TODOs;
- commented-out implementation.

Comment cleanup must not change behavior.

## A4. Known Issues workbook

During the scan, prepare the official Assignment-3 Known Issues workbook.

- [ ] Preserve the official workbook structure and allowed values.
- [ ] Edit only the `Known Issues` sheet.
- [ ] Delete the example rows.
- [ ] Record only **genuine unresolved issues that will remain in the submitted project**.
- [ ] Do not record an issue after it is fixed.
- [ ] Do not invent issues just to fill the file.
- [ ] Never include test-only issues.
- [ ] Put the student IDs in the workbook filename as requested by the template.

## Required PART A report

Return only:

### FIX NOW
Sorted from highest submission risk to lowest:

`priority | error code if applicable | file:line | problem | minimal recommended fix`

### KNOWN ISSUES TO SUBMIT
Every unresolved issue placed in the workbook.

### COMMENT CLEANUP
Only the production files whose comments were changed.

Do not include learning explanations, feature inventories, or optional refactoring suggestions.
